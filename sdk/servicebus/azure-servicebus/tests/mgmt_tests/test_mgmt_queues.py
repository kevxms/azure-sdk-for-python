...
class TestServiceBusAdministrationClientQueue(AzureMgmtRecordedTestCase):
    @ServiceBusPreparer()
    @recorded_by_proxy
    def test_mgmt_queue_list_basic(
        self,
    ...
    def test_mgmt_queue_list_with_negative_credential(
        ...
        mgmt_service = ServiceBusAdministrationClient(
            servicebus_fully_qualified_namespace, credential=ServiceBusSharedKeyCredential("invalid", "invalid")
        )
        with pytest.raises(HttpResponseError):
            list(mgmt_service.list_queues())
    ...
    def test_mgmt_queue_create_with_queue_description(self, servicebus_fully_qualified_namespace, **kwargs):
        ...
        try:
            ...
            assert (
                queue.max_size_in_megabytes % 3072 == 0
            )  # TODO: In my local test, I don't see a multiple of the input number. To confirm
    ...
    def test_mgmt_queue_premium_create_with_queue_description(self, servicebus_fully_qualified_namespace, **kwargs):
        ...
        with pytest.raises(HttpResponseError):
            mgmt_service.create_queue(queue_name_3, max_message_size_in_kilobytes=1023)  # min allowed is 1024

        with pytest.raises(HttpResponseError):
            mgmt_service.create_queue(queue_name_3, max_message_size_in_kilobytes=102401)  # max allowed is 102400
    ...
    @ServiceBusPreparer()
    @recorded_by_proxy
    def test_mgmt_queue_basic_v2017_04(
